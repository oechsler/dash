...
func (uc *UpdateUserCategory) Execute(ctx context.Context, userId string, in UpdateUserCategoryInput) error {
	...
	if err != nil {
		return Internal("update user category: get category", err)
	}
	if category == nil {
	...
	if err != nil {
		return Internal("update user category: get dashboard", err)
	}
	if dashboard == nil {
	...
	if err := uc.CategoryRepo.Upsert(ctx, category); err != nil {
		return Internal("update user category: upsert", err)
	}
	return nil
}
